...
<dict>
    ...
    <string>${projectRoot}/logs/retn0claw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/retn0claw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.retn0claw');
  } catch {
...
/**
 * Kill any orphaned retn0claw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned retn0claw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/retn0claw.service';
    ...
    unitPath = path.join(unitDir, 'retn0claw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=retn0claw Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/retn0claw.log
StandardError=append:${projectRoot}/logs/retn0claw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned retn0claw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable retn0claw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start retn0claw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active retn0claw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-retn0claw.sh');
  const pidFile = path.join(projectRoot, 'retn0claw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-retn0claw.sh — Start retn0claw without systemd',
    ...
    '    echo "Stopping existing retn0claw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting retn0claw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/retn0claw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/retn0claw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "retn0claw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/retn0claw.log"`,
  ];
