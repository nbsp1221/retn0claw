...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.retn0claw')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.retn0claw'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active retn0claw`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'retn0claw', 'mount-allowlist.json'),
    )
  ) {
