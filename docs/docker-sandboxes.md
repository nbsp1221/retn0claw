# Running retn0claw in Docker Sandboxes (Manual Setup)

This guide walks through setting up retn0claw inside a [Docker Sandbox](https://docs.docker.com/ai/sandboxes/) from scratch — no install script, no pre-built fork. You'll clone the upstream repo, apply the necessary patches, and have agents running in full hypervisor-level isolation.
...
Enter the sandbox:
```bash
docker sandbox run shell-retn0claw-workspace
```
...
## Step 3: Clone and Install retn0claw

retn0claw must live inside the workspace directory — Docker-in-Docker can only bind-mount from the shared workspace path.
...
WORKSPACE=/Users/you/retn0claw-workspace

# Move into workspace so DinD mounts work
mv retn0claw "$WORKSPACE/retn0claw"
cd "$WORKSPACE/retn0claw"
...
retn0claw needs several patches to work inside a Docker Sandbox. These handle proxy routing, CA certificates, and Docker-in-Docker mount restrictions.
...
In `src/container-runtime.ts`, the `cleanupOrphans()` function matches containers by the `retn0claw-` prefix. Inside a sandbox, the sandbox container itself may match (e.g., `retn0claw-docker-sandbox`). Filter out the current hostname:
...
```bash
cd ~ && git clone https://github.com/nbsp1221/retn0claw.git && mv retn0claw /path/to/workspace/retn0claw
```
