...
describe('codex app-server client', () => {
  ...
  it('streams multiple progress deltas and ignores late deltas after turn completion', async () => {
    const harness = createHarness();
    ...
    harness.send({
      method: 'item/agentMessage/delta',
      params: { threadId: 'thread-1', turnId: 'turn-1', delta: 'ab' },
    });
    harness.send({
      method: 'item/completed',
      params: {
    ...
    expect(onProgress).toHaveBeenNthCalledWith(1, 'a', 'turn-1');
    expect(onProgress).toHaveBeenNthCalledWith(2, 'ab', 'turn-1');
  });

  it('completes with a null result when turn/completed arrives without an agent message item', async () => {
    const harness = createHarness();
