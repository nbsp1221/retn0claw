...
import {
  createCodexObservability,
  type CodexObservabilityOptions,
...
export interface CodexAppServerClientOptions {
  ...
  observability?: CodexObservabilityOptions;
}
...
export class CodexAppServerClient {
  ...
  private readonly observability: ReturnType<
    typeof createCodexObservability
  > | null;
  ...
  constructor(options: CodexAppServerClientOptions) {
    ...
    this.observability = options.observability
      ? createCodexObservability(options.observability)
      : null;
    this.spawnAppServer =
  ...
  async start(): Promise<void> {
    ...
    this.proc.stdout.on('data', (chunk: string) => {
      ...
      for (const line of lines) {
        ...
        if (!trimmed) continue;
        this.observability?.observeRawStdoutLine(trimmed);
    ...
    this.proc.stderr.on('data', (chunk: string) => {
      for (const line of chunk.split('\n')) {
        const trimmed = line.trim();
        if (trimmed) {
          this.observability?.observeRawStderrLine(trimmed);
  ...
  async startOrResumeThread(
    ...
    this.observability?.applyEvent({ type: 'thread.started', threadId });
    return threadId;
  }
  ...
  private handleMessage(line: string): void {
    ...
    try {
      ...
      this.observability?.onParseFailure(
        line,
    ...
    this.observability?.applyParsedNotification(message);
    ...
    if (
      ...
      this.activeTurn.onProgress?.(
        String(message.params?.delta || ''),
  ...
  recordHostEvent(
    ...
    this.observability?.recordHostEvent(payload, ids);
  }

  emitRunStarted(details?: Record<string, unknown>): void {
    this.observability?.emitRunStarted(details);
  }

  emitSessionResumeAttempted(sessionId: string): void {
    this.observability?.emitSessionResumeAttempted(sessionId);
  }

  emitSessionResumeFailed(sessionId: string, error: string): void {
    this.observability?.emitSessionResumeFailed(sessionId, error);
  }
  ...
  emitSessionReplaced(
    ...
    this.observability?.emitSessionReplaced(previousSessionId, nextSessionId);
  }
  ...
  emitSessionStuck(details: {
    ...
    this.observability?.emitSessionStuck(details);
  }
}
